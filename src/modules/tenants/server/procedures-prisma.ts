  ...
  getOne: baseProcedure
    ...
    .query(async ({ input }) => {
      ...
      try {
        ...
        console.log("[tenantsRouter.getOne] Query result:", tenant ? tenant.name : "Not found");
  ...
  discoverNearby: baseProcedure
    ...
    .query(async ({ input }) => {
      ...
      const tenants = await prisma.tenant.findMany({
        where,
        take: 1000, // Get all matching tenants for distance calculation
      });

      // Calculate distances and filter by delivery radius
      const nearbyTenants = tenants
        .map(tenant => {
          ...
          if (currentlyDelivering && tenant.operatingHours) {
            isCurrentlyOpen = checkIfCurrentlyOpen(tenant.operatingHours as any);
          }
    ...
    .query(async ({ input }) => {
      ...
      const [tenants, totalCount] = await Promise.all([
        prisma.tenant.findMany({
          where,
          skip: offset,
          take: limit + 20, // Get extra in case some are filtered out by distance
          orderBy: {
            createdAt: 'desc',
          },
        }),
      ...
      let processedTenants = tenants.map(tenant => {
